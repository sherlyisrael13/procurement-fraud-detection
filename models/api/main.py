...
def get_shap_reasons(shap_vals: np.ndarray, feature_names: list, top_n: int = 3) -> list:
...
def predict(order: OrderRequest):
    try:
        ...
        feature_vector = np.array([[
            order.amount,
            amount_ratio,
            order.vendor_frequency,
            vendor_rare,
            order.approval_lag_hours,
            order.approved_on_weekend,
            dept_risk,
            dept_enc,
            cat_enc
        ]])
        ...
        conn.execute(
            ...
            (order.order_id, is_anomaly, anomaly_score,
             risk_level, result["timestamp"])
        )
        conn.commit()
        conn.close()
...
def submit_feedback(feedback: FeedbackRequest):
    if feedback.label not in ["true_fraud", "false_positive"]:
        raise HTTPException(
            status_code=400,
            detail="label must be 'true_fraud' or 'false_positive'"
        )
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS feedback (
                order_id TEXT, label TEXT,
                reviewer_note TEXT, timestamp TEXT
            )
        """)
        conn.execute(
...
def retrain():
    try:
        ...
        return {
            ...
            "message": "Submit feedback via /feedback first."
        }