...
<style>
    ...
    /* Risk alert boxes */
    .risk-critical {
        background: #fff0f0; border-left: 5px solid #d62828;
        border-radius: 8px; padding: 1rem 1.25rem; margin: 1rem 0;
    }
    .risk-high {
        background: #fff7f0; border-left: 5px solid #f77f00;
        border-radius: 8px; padding: 1rem 1.25rem; margin: 1rem 0;
    }
    .risk-medium {
        background: #fffbf0; border-left: 5px solid #f4c430;
        border-radius: 8px; padding: 1rem 1.25rem; margin: 1rem 0;
    }
    .risk-low {
        background: #f0fff4; border-left: 5px solid #2dc653;
        border-radius: 8px; padding: 1rem 1.25rem; margin: 1rem 0;
    }
    .risk-title {
    ...
    .metric-blue  { border-left: 4px solid #4361ee !important; }
    .metric-red   { border-left: 4px solid #d62828 !important; }
    .metric-amber { border-left: 4px solid #f77f00 !important; }
    .metric-green { border-left: 4px solid #2dc653 !important; }

    /* Footer */
    .footer {
        text-align: center; padding: 2rem 0 1rem;
        font-size: 12px; color: #aaa;
        border-top: 1px solid #eaeaea; margin-top: 2rem;
    }
</style>
...
def risk_alert(risk_level, order_id, score):
    ...
    st.markdown(f"""
    <div class="{cls}">
        <div class="risk-title">{icon} {risk_level} Risk — {order_id}</div>
        <div class="risk-desc">{desc.get(risk_level,'')} &nbsp;|&nbsp; Anomaly Score: {score}</div>
    </div>
    """, unsafe_allow_html=True)
...
if page == "📊 Dashboard Overview":
    ...
    with col1:
        st.metric("Total Orders", f"{total:,}", help="Total purchase orders in dataset")
    with col2:
        st.metric("Anomalies Found", f"{total_anom:,}", delta=f"{anom_pct}% rate", delta_color="inverse")
    with col3:
        st.metric("Anomaly Rate", f"{anom_pct}%", help="Percentage of orders flagged")
    with col4:
        st.metric("API Predictions", f"{api_preds:,}", help="Orders analysed via API")
    ...
    with col_l:
        ...
        fig = px.bar(
            anom_df, x='Type', y='Count',
            color='Count',
            color_continuous_scale=[[0,'#fde8e8'],[1,'#d62828']],
            text='Count'
        )
    ...
    fig3.update_layout(
        plot_bgcolor='white', paper_bgcolor='white',
        height=280, margin=dict(t=10,b=40,l=20,r=20),
        xaxis=dict(title='Amount (₹)', gridcolor='#f0f0f0'),
        yaxis=dict(title='Count', gridcolor='#f0f0f0'),
        legend=dict(title='')
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("predict_form"):
        ...
        submitted = st.form_submit_button("🔍 Analyse Order")
    ...
    if submitted:
        ...
        with st.spinner("Running anomaly analysis..."):
            try:
                ...
                st.caption(
                    "Negative contribution = feature pushed the score toward anomalous. "
                    "Larger absolute value = stronger influence on the flagging decision."
                )
                ...
                fig.update_layout(
                    ...
                    xaxis=dict(title='Contribution', gridcolor='#f0f0f0'),
                    yaxis=dict(title='')
                )
                st.plotly_chart(fig, use_container_width=True)
                ...
                if st.button("Submit Feedback"):
                    ...
                    if fb_res.status_code == 200:
                        st.success("Feedback recorded successfully. Thank you.")
                    else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        m1.metric("Precision",     metrics['precision'], help="Of all orders flagged, how many were true anomalies")
        m2.metric("Recall",        metrics['recall'],    help="Of all true anomalies, how many were caught")
        m3.metric("F1 Score",      metrics['f1'],        help="Harmonic mean of precision and recall")
        m4.metric("Total Orders",  f"{metrics['total_orders']:,}")

        st.caption(
            "Metrics computed on injected ground-truth anomaly labels. "
        ...
        st.caption(
            "Average absolute SHAP contribution per feature across all orders. "
            "Higher value = stronger influence on the anomaly score."
        )
        ...
        fig.update_layout(
            ...
            xaxis=dict(title='Mean |SHAP| Value', gridcolor='#f0f0f0'),
            yaxis=dict(title='')
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        st.dataframe(pd.DataFrame({
            'Feature': [
                'amount','amount_to_category_ratio','vendor_frequency',
