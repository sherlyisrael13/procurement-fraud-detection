...
RISK_COLORS = {'Critical':'#d62828','High':'#f77f00','Medium':'#f4c430','Low':'#2dc653'}
...
if page == "📊 Dashboard Overview":
    ...
    if st.button("🔍 Analyse Order"):
        ...
        with st.spinner("Running anomaly analysis..."):
            try:
                ...
                with fb2:
                    note = st.text_input("Reviewer Note (optional)", placeholder="e.g. Known vendor")
                if st.button("Submit Feedback"):
                    fb_res = requests.post(f"{API_URL}/feedback", json={"order_id": order_id, "label": label, "reviewer_note": note})
    ...
    try:
        ...
        if data['total'] == 0:
            ...
            st.dataframe(
                pred_df.style.map(
                    lambda v: f"color: {RISK_COLORS.get(v,'#333')}; font-weight:600" if v in RISK_COLORS else '',
    ...
    try:
        ...
        shap_df = pd.DataFrame(list(shap_imp.items()), columns=['Feature','Importance']).sort_values('Importance', ascending=True)
        fig = go.Figure(go.Bar(
